...
- **LangGraph agent** (`app/agents/graph.py`): a 4-node graph — `planner` → (conditionally) `retriever` → `evidence_agent` → `responder` → END, compiled with `PostgresSaver` checkpointed by `thread_id` for per-conversation memory (`settings.DATABASE_URL`) — durable across backend restarts, unlike the old in-memory `MemorySaver`. State shape is `AgentState` (`app/agents/state.py`).
