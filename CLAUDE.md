...
- **Guardrails gate first** (`app/guardrails/rails.py`, rules in `colang_rules.py`): every query hits this before the agent. If a rail fires, `main.py` returns the guardrail's response immediately and the LangGraph agent never runs. Uses a small/fast Groq model (`allam-2-7b`) for intent classification, distinct from the main RAG model.
